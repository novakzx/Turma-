...
App mobile pra estudantes do ensino fundamental/médio: mural de avisos da escola, ferramentas de estudo (chat com IA, calculadora de notas) e uma comunidade da turma (feed + chat), tudo num só lugar.
...
```
app/                  # Rotas (Expo Router) — só navegação e composição de tela
src/
  features/           # Uma pasta por domínio: auth, avisos, estudo, notas, feed, chat, perfil
...
- `papel`, `escola_id` e `turma_id` do próprio perfil não são editáveis pelo usuário (só por quem administra o banco) — evita autopromoção pra professor/coordenacao.
...
- [ ] **Fase 1 — Conta e perfil**: cadastro/login, onboarding de escola/turma, papéis.
...
Fase 0 completa: app roda (`npm start`), NativeWind renderiza a paleta acima, schema versionado, lint/typecheck/test passando localmente e no CI. Fase 1 é o próximo passo.
