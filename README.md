...
- **Barra de abas flutuante pode tampar conteúdo colado na base da tela**: `tabBarStyle` é `position: absolute` (visual "flutuante" da Fase 6 — ver `(tabs)/_layout.tsx`), então qualquer tela dentro das 6 abas que gruda algo na base via flex (só `estudo.tsx` faz isso, a linha de enviar mensagem) precisa reservar o espaço da barra (`pb-24`) ou o toque no botão de baixo nem chega a ele. Achado testando o chat com a Gemini de ponta a ponta pela primeira vez — o clique em "Enviar" silenciosamente não fazia nada porque a barra de abas estava por cima do botão. Vale conferir de novo se alguma tela nova dentro de `(tabs)` grudar conteúdo na base.
