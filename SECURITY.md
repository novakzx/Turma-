...
| `Cache-Control`             | `no-store, must-revalidate`                             | Nenhuma resposta fica em cache (CDN da Vercel ou navegador) — ver justificativa abaixo                               |
...
**`Cache-Control: no-store` em tudo, e por quê**: achado ao vivo depois de corrigir a CSP pra liberar `blob:` (ver abaixo) — o deploy novo tinha ido pro ar (confirmado por `fetch()` direto), mas navegações reais continuavam recebendo a CSP **antiga** por vários minutos, de forma inconsistente entre requisições da mesma sessão (`X-Vercel-Cache: HIT` num nó do edge global já com o conteúdo novo, enquanto a navegação de verdade batia num nó ainda desatualizado). Como todo este projeto é servido via rewrite de SPA (`"/(.*)" → "/index.html"`, ver `rewrites` acima), esse é o único documento HTML da produção inteira — sem `Cache-Control` explícito, a Vercel escolhia `public, max-age=0, must-revalidate` por padrão, o que já devia forçar revalidação, mas na prática não impediu esse período de inconsistência entre nós do CDN. `no-store` elimina a ambiguidade de vez: toda resposta (HTML e os poucos outros arquivos que passam por aqui) é sempre buscada fresca da origem, nunca fica em cache em lugar nenhum. Custo aceito: um pouco menos de cache pros assets estáticos (JS/CSS do build, que ainda ganham cache de navegador via ETag/condicional normal) — pra uma escola, a garantia de nunca mais servir versão desatualizada vale mais que essa otimização.
