...
| `Cache-Control`             | `no-store, must-revalidate` no HTML; `public, max-age=31536000, immutable` no JS/CSS/fonte com hash | HTML nunca fica em cache (CDN da Vercel ou navegador); os arquivos estáticos com hash no nome cacheiam pra sempre — ver justificativa abaixo |
...
**`Cache-Control: no-store` no HTML, e por quê**: achado ao vivo depois de corrigir a CSP pra liberar `blob:` (ver abaixo) — o deploy novo tinha ido pro ar (confirmado por `fetch()` direto), mas navegações reais continuavam recebendo a CSP **antiga** por vários minutos, de forma inconsistente entre requisições da mesma sessão (`X-Vercel-Cache: HIT` num nó do edge global já com o conteúdo novo, enquanto a navegação de verdade batia num nó ainda desatualizado). Como todo este projeto é servido via rewrite de SPA (`"/(.*)" → "/index.html"`, ver `rewrites` acima), esse é o único documento HTML da produção inteira — sem `Cache-Control` explícito, a Vercel escolhia `public, max-age=0, must-revalidate` por padrão, o que já devia forçar revalidação, mas na prática não impediu esse período de inconsistência entre nós do CDN. `no-store` no HTML elimina a ambiguidade de vez: o documento é sempre buscado fresco da origem, nunca fica em cache em lugar nenhum.

**Correção depois, achada ao vivo de novo (pedido do usuário — "demora muito pra carregar")**: a regra original acima ("`no-store` em tudo") aplicava esse MESMO cabeçalho a `/(.*)`, sem exceção — incluindo o bundle JS/CSS com hash no nome (`_expo/static/js/web/entry-<hash>.js`, ~3,5 MB). A suposição de que esses arquivos "ainda ganhavam cache de navegador via ETag/condicional normal" estava **errada**: `no-store` proíbe qualquer cache, inclusive a revalidação condicional (304) — o navegador não tinha alternativa a não ser rebaixar o bundle inteiro do zero em toda visita, mesmo repetida, mesmo sem nada ter mudado. Confirmado ao vivo direto na produção (`fetch()` real contra `suaturma.vercel.app`, `Cache-Control: no-store, must-revalidate` no bundle inteiro). Esse arquivo é justamente o tipo que É seguro cachear pra sempre: o nome já tem o hash do conteúdo — se o conteúdo mudar, o próximo build gera um nome novo, então nunca existe risco de servir uma versão velha com nome de uma versão nova (o problema original, específico do HTML, nunca existiu aqui). Corrigido com duas regras novas em `vercel.json`, depois da regra geral (a Vercel aplica por último quem bate por último quando a mesma chave de cabeçalho se repete): `/_expo/static/(.*)` e `/assets/(.*)` (onde ficam JS/CSS/fontes do build, todos com hash no nome) ganham `public, max-age=31536000, immutable`. O HTML continua `no-store` — a garantia contra CSP desatualizada não foi tocada, só o custo desnecessário em cima dos arquivos que nunca tiveram esse risco.

**CSP escolhida e por quê**: `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://njzstudshifdjdqwbqsa.supabase.co; font-src 'self' data:; connect-src 'self' blob: https://njzstudshifdjdqwbqsa.supabase.co wss://njzstudshifdjdqwbqsa.supabase.co; media-src 'self' blob: https://njzstudshifdjdqwbqsa.supabase.co; object-src 'none'; base-uri 'none'; form-action 'self'; frame-ancestors 'none'; upgrade-insecure-requests`.
...
- `connect-src` também libera `blob:` — achado ao vivo em produção (não no dev server, que não aplica CSP nenhuma): todo upload de foto no navegador (post, story, chat) lê os bytes do arquivo escolhido com `fetch(uri)` antes de mandar pro Storage, e no alvo **web** essa `uri` é um `blob:...` criado pelo próprio `expo-image-picker` — sem `blob:` em `connect-src`, esse `fetch` (que o navegador trata como uma requisição de rede comum, mesmo sendo 100% local/mesma-origem) é bloqueado com "Failed to fetch"/"Load failed", sem nenhuma pista de CSP na mensagem de erro que chega no app. `img-src`/`media-src` já tinham `blob:` (necessário só pra **exibir** `<img>`/áudio a partir de um blob local), mas isso não cobre **ler** o blob via `fetch` — são diretivas diferentes da mesma CSP. Não é abertura de superfície real: um `blob:` só existe na memória da própria aba que o criou, não é um destino de rede externo.
